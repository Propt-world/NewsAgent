...
-   **Resilient Queue System**: Features a Dead Letter Queue (DLQ) for failed jobs and status tracking for every request
-   **Error Notifications**: Automatically sends email alerts with error details and tracebacks to administrators when jobs fail
...
1.  **API Server** (`src/main.py`): FastAPI application that accepts job submissions, manages queues, and provides system visibility.
2.  **Worker** (`src/worker.py`): Background process that consumes jobs from Redis, executes the LangGraph workflow, handles errors, and manages the DLQ.
3.  **Redis**: Message broker that manages the Main Queue, Dead Letter Queue, and Job Status tracking.
...
3.  **Configure environment variables**: Edit `.env` and fill in your API keys and SMTP settings:
    ...
    # Webhook & Database
    WEBHOOK_URL=[https://your-webhook-endpoint.com](https://your-webhook-endpoint.com)
...
4.  **Initialize the database**:
    ...
    ```
    _This creates tables and adds a default admin recipient for error emails._
...
7.  **Configure environment variables**: Edit `.env` with your settings (API Keys, SMTP, Redis URL).
...
### Queue & DLQ Management API

The system exposes endpoints to monitor and manage queues:

**1. Check Queue Counts (Main & DLQ):**
```
curl http://localhost:8000/queue/status
```

**2. List Items in Main Queue:**
```
curl "http://localhost:8000/queue/main/items?limit=5&offset=0"
```

**3. List Items in Dead Letter Queue (DLQ):**

```
curl "http://localhost:8000/queue/dlq/items?limit=5&offset=0"
```

**4. Requeue a Specific Failed Job (DLQ -> Main):**

```
curl -X POST "http://localhost:8000/queue/dlq/requeue/{job_id}"
```

**5. Requeue All Failed Jobs:**

```
curl -X POST "http://localhost:8000/queue/dlq/requeue-all"
```
...
Redis queue name for failed jobs
...
SMTP Server (e.g. https://www.google.com/search?q=smtp.gmail.com)
...
Prompt templates are stored in the database and can be managed through the `PromptTemplate` model. The initialization script (`src/scripts/init_db.py`) seeds the database with default prompts.
...
│   ├── db/
│   │   ├── models.py            # SQLAlchemy models (Prompts, EmailRecipients)
...
│   ├── worker.py                # Worker process & DLQ Logic
...
-   **Never commit `.env` files**: They contain sensitive API key
