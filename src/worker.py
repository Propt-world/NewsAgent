...
async def run_worker():
    ...
    try:
        ...
        print(f"--- 👷 NewsAgent Worker Started (Async) ---")
        ...
        try:
            ...
            try:
                ...
                if error_message:
                    ...
                    # Send Email (Sync function, but acceptable here)
                    send_error_email(
                        job_id=job_id,
                        source_url=source_url,
                    ...
                    await update_job_status(r, job_id, "completed", result=article_data)
            ...
            traceback.print_exc()
            await asyncio.sleep(1) # Prevent hot loop on repeated error
