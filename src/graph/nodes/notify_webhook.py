...
async def notify_webhook(state: MainWorkflowState) -> MainWorkflowState:
    """
    ...
    try:
        ...
        if response.status_code in [200, 201, 202]:
            pprint("[NODE: NOTIFY WEBHOOK] Success! Downstream service accepted data.")
        else:
            pprint(
                f"[NODE: NOTIFY WEBHOOK] Warning: Service returned {response.status_code}: {response.text}"
            )
